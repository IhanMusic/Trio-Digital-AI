...
router.put('/:id', authenticate, upload.single('logo'), async (req: express.Request, res: express.Response) => {
  try {
    ...
    console.log('🔍 Début mise à jour marque...');
    console.log('📁 Fichier reçu:', req.file ? 'Oui' : 'Non');
    console.log('📝 Body reçu:', Object.keys(req.body));
    ...
    console.log('💾 Mise à jour de la marque en base...');
    console.log('📊 Données finales:', {
      name: brandData.name,
      sector: brandData.sector,
      hasDescription: !!brandData.description,
    ...
    if (!brand) {
      console.error('❌ Marque non trouvée:', req.params.id);
      return res.status(404).json({
        success: false,
        message: 'Marque non trouvée'
      });
    ...
    console.log('✅ Marque mise à jour avec succès:', brand._id);
    ...
    let statusCode = 400;
    let message = 'Erreur lors de la mise à jour de la marque';
    let details = '';
    let errorCode = 'UNKNOWN_ERROR';
