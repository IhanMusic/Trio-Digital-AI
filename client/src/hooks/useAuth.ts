...
export const useAuth = () => {
  ...
  const updateAuthState = async (user: User | null, token: string | null, refreshToken: string | null) => {
    if (token && user) {
  ...
  const refreshToken = async () => {
    try {
      ...
      if (response.ok) {
        ...
        await updateAuthState(auth.user, data.data.accessToken, auth.refreshToken);
        return true;
      }
      return false;
    } catch (error) {
  ...
  const changePassword = async (currentPassword: string, newPassword: string) => {
    try {
      ...
      return { 
        success: false, 
        error: data.error || 'Une erreur est survenue lors du changement de mot de passe' 
      };
    } catch (error) {
  ...
  const updateProfile = async (name: string, email: string) => {
    try {
      ...
      if (response.ok) {
        ...
        return { 
          success: false, 
          error: data.error || 'Une erreur est survenue lors de la mise à jour du profil' 
        };
      }
    } catch (error) {
  ...
  const updateUser = async (updates: Partial<User>) => {
    try {
      ...
      const response = await fetch(`${config.apiUrl}/auth/update-user`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${auth.token}`
